...
def get_eur_usd_rate():
    ...
    logger.error("Impossible de récupérer le taux EUR/USD")
    return 1.05
...
def afficher_montant_double(label, montant_usd, delta_str="", couleur_valeur=None, taille="large"):
    try:
        ...
        t_val, t_lbl = {
            "large": ("1.8rem", "0.9rem"),
            "medium": ("1.4rem", "0.85rem"),
            "small": ("1.2rem", "0.85rem")
        }.get(taille, ("1.8rem", "0.9rem"))
        ...
        logger.error(f"Erreur affichage montant: {e}")
...
def recalculer_totaux_locaux():
    ...
    try:
        ...
        logger.error(f"Erreur recalcul totaux: {e}")
...
def actualiser_cours_internet(silencieux=False):
    ...
    for _, row in df_tmp.iterrows():
        ...
        if not tick or tick in ["NAN", "USD"] or tick.endswith("USDT"):
            continue
    ...
    if yf_tickers_to_fetch:
        ...
        if data is not None:
            ...
            st.toast("⚠️ Impossible de joindre Yahoo Finance.", icon="⚠️")
    ...
    for idx, row in df_tmp.iterrows():
        ...
        if yf_t and yf_t in hist_data:
            ...
            if tick in ["EUR", "CHF", "JPY", "GBP", "CNY", "CAD", "AUD"]:
                ...
                df_tmp.at[idx, "Court"] = format_smart(p_usd, "$", is_price=True)
            changement = True
        elif tick not in st.session_state.variations:
            st.session_state.variations[tick] = "→ 0.00 %"
    ...
    if "config" not in st.session_state:
        try:
            ...
            def parse_config_val(k, v):
                ...
                if k_str in ["f_statut", "urssaf_bareme", "f_pays_etr"]:
                    return v_str
                if k_str in ["f_u1", "f_u2"]:
                    return v_str.lower() in ['true', '1', '1.0', 'oui', 'yes']
...
if page_choisie == "📊 Tableau de bord":
    ...
    with c_stat:
        if besoin_req:
            st.warning("⚠️ **Rééquilibrage nécessaire**")
        else:
    ...
    with st.expander("➕ Enregistrer une transaction"):
        with st.form("new_trans"):
            ...
            if st.form_submit_button("🔨 Valider"):
                try:
                    if t_t.strip() == "" or t_q <= 0 or t_c <= 0:
    ...
    with c_p2:
        rendement_a = st.number_input("Scénario A (%) ✍️", min_value=0.00, max_value=30.00, value=round(max(0.00, float(moy_brute_hist)), 2), step=0.01)
    ...
    with c_sit1:
        statut = st.radio("Situation matrimoniale ✍️", ["Célibataire / Divorcé(e) / Veuf(ve)", "Marié(e) / Pacsé(e)"], index=0 if st.session_state.config.get("f_statut") == "Célibataire / Divorcé(e) / Veuf(ve)" else 1, key="in_statut", on_change=update_fiscal_config)
    with c_sit2:
        enfants = st.number_input("Enfants à charge ✍️", min_value=0, max_value=10, value=int(st.session_state.config.get("f_enf", 0)), step=1, key="in_enf", on_change=update_fiscal_config)
    st.divider()
    ...
    if bilan_net_actions > 0:
        ...
        if choix == "Barème":
            st.success(f"✅ **Barème Progressif** plus avantageux : **{format_smart(cout_bareme, '€')}** (Taux effectif : {format_smart(taux_moyen_bareme, '%')})")
        else:
            st.success(f"✅ **Flat Tax (PFU)** plus avantageuse : **{format_smart(cout_pfu, '€')}**")
