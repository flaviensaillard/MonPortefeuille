...
if page_choisie == "📊 Tableau de bord":
    ...
    with col_btn:
        if st.button("🔄 Actualiser les cours", use_container_width=True):
            ...
            st.rerun()
    with col_statut:
        if besoin_reequilibrage: st.warning("⚠️ **Rééquilibrage nécessaire** (Certains actifs ont dépassé les tolérances. Consultez l'onglet Rééquilibrage.)")
        else: st.success("✅ **Équilibré** (Votre stratégie d'allocation cible est actuellement respectée.)")
    st.divider()
    ...
    with c3:
        ...
        if ecart == 0: info_str, color_info = "✅ Cible atteinte", "#2ecc71"
        elif ecart > 0: info_str, color_info = f"⚠️ {ecart:.2f} % manquant", "#e74c3c"
        else: info_str, color_info = f"⚠️ {abs(ecart):.2f} % en trop", "#e74c3c"
        html_repartition = f"""
        <div style="margin-bottom: 0.8rem;">
            <div style="font-size: 0.9rem; opacity: 0.8; margin-bottom: 0.2rem;">Répartition Cible</div>
            <div style="font-size: 1.8rem; font-weight: 600; line-height: 1.2;">{somme_p:.2f} %</div>
    ...
    if df_p.empty: st.info("Aucune donnée disponible. Le premier point sera enregistré cette nuit.")
        ...
        if jours_annee_1 > 0 and jours_annee_1 < 330:
            st.info(f"💡 **Note :** Votre année de lancement ({annee_debut_absolue}) ayant duré moins d'un an, son pourcentage de rentabilité a été **annualisé** (projeté mathématiquement sur un rythme de 12 mois complets). Cela permet de l'intégrer à vos moyennes et de la comparer à l'inflation.")
    ...
    with c_p1:
        ...
        apport_mensuel = st.number_input("Apport mensuel d'aujourd'hui ($) ✍️", min_value=0.00, step=50.00, value=float(st.session_state.config.get("retraite_apport_mensuel", 250.0)), key="ret_app", on_change=on_retraite_params_change)
    with c_p2:
    ...
    with c_p3:
        ...
        taxe_plus_value = st.number_input("Fiscalité sur les retraits (Flat Tax) (%) ✍️", min_value=0.00, max_value=60.00, step=0.10, value=float(st.session_state.config.get("retraite_taxe", 30.0)), key="ret_tax", on_change=on_retraite_params_change)
