...
oui mais je veux également mon taux personnalisé. et le quotient familliale tu le sort d'où? il est donné par la CAF normalement
Nous devons ajouter le taux personnalisé (taux de prélèvement à la source individuel) et clarifier l'origine du quotient familial. Le quotient familial est bien une donnée administrative (CAF ou calcul selon situation). Dans la demande précédente, l'utilisateur voulait une case pour le quotient familial mémorisée, ce que nous avons fait avec in_parts. Mais il faut aussi afficher le taux personnalisé pour chaque déclarant, comme précédemment, mais avec un calcul correct. L'ancien code calculait un taux personnalisé approximatif basé sur le revenu d'un seul déclarant avec abattement et situation célibataire, ce qui n'était pas exact. Pour avoir un taux personnalisé réaliste, il faut diviser l'impôt du foyer par le revenu global ou utiliser le taux moyen du foyer (c'est ce que le fisc utilise pour le taux de PAS par défaut). Mais l'utilisateur veut peut-être un taux personnalisé pour chaque conjoint. On peut afficher le taux moyen du foyer (c'est le taux de PAS individualisé par défaut si les conjoints ne demandent pas de taux différencié) et éventuellement indiquer comment obtenir un taux personnalisé (sur le site des impôts). Toutefois, pour répondre à sa demande spécifique, on peut afficher un taux personnalisé estimé pour le déclarant 1 et 2 en utilisant la méthode de l'administration (impôt total / revenus totaux). C'est le taux neutre. Si on veut un taux personnalisé qui tient compte des disparités de revenus, il faut faire une simulation de l'impôt individuel, ce qui est complexe. Je vais opter pour afficher le taux moyen du foyer comme taux de PAS, et ajouter une note indiquant que c'est le taux individualisé par défaut. Je supprimerai l'ancien calcul du taux personnalisé inexact. Mais l'utilisateur a dit "je veux également mon taux personnalisé", donc il faut lui donner quelque chose. Je vais expliquer que le taux personnalisé est égal au taux moyen du foyer, et que s'il veut un taux différencié, il peut le calculer sur le site des impôts. Je vais donc retirer le calcul de taux_perso_1 erroné et mettre simplement le taux moyen du foyer comme taux de PAS.

De plus, je dois m'assurer que le code de la page Fiscalité fourni est complet et fonctionnel, et qu'il n'y a pas d'erreur d'indentation ou de syntaxe. Je vais réécrire la section entière de la page Fiscalité avec ces modifications, en incluant le champ parts, le bilan fiscal amélioré, et le taux moyen comme taux de PAS.

Je vais également vérifier que le update_fiscal_config inclut bien in_parts.

Maintenant, je vais fournir le code complet de la page Fiscalité.

Voici la page Fiscalité complète, incluant la gestion du quotient familial mémorisé, le bilan fiscal précis, et le taux de prélèvement à la source (taux personnalisé) basé sur le taux moyen du foyer.
...
python
    ...
    # Calcul du nombre de parts automatique (rappel)
    parts_auto = 1.0 if "Célibataire" in statut else 2.0
    if enfants == 1:
        parts_auto += 0.5
    elif enfants == 2:
    ...
    st.subheader("💡 3. Recommandation")
    if bilan_net_actions > 0:
        taux_moyen_bareme = (cout_bareme / bilan_net_actions) * 100 if bilan_net_actions > 0 else 0
        if choix == "Barème":
            st.success(f"✅ **Le Barème Progressif est plus avantageux pour vos plus-values !**")
            st.write(f"Avec le Barème : l'impôt est de **{format_smart(cout_bareme, '€')}** *(Taux effectif : {format_smart(taux_moyen_bareme, '%')} )*.")
        else:
    ...
    qf = revenu_base_net_global / parts if parts > 0 else 0
    taux_marginal = "0%"
    for _, _, _, label in tranches:
        if qf > _:
            taux_marginal = label
