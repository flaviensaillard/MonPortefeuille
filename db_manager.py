...
def obtenir_derniere_projection_veille():
    ...
    try:
        ...
        if df_proj is not None and not df_proj.empty:
            ...
            df_proj['Date_Parsed'] = pd.to_datetime(
                df_proj['Date'].astype(str).str.slice(0, 10),
                dayfirst=True,
                errors='coerce'
            )
...
def recalculer_toute_la_base_projections(df):
    ...
    df['Evolution cumulée %'] = np.where(
        df['Capital investi'] > 0,
    ...
    df['TG_Evolution cumulée %'] = np.where(
        df['Capital investi'] > 0,
